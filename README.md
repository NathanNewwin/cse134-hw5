...
The site initially supports light and dark modes by using `color-scheme` and `prefers-color-scheme` in the CSS. This means that if JavaScript is disabled, the page still follows the user's system setting and stays fully usable (user can still change themes by changing browsers theme setting). I also added a `<noscript>` message to explain that the user will not be able to manually override the color scheme through the theme picker when JavaScript is disabled. The theme picker markup is hidden by default, then the JavaScript unhides and wires a small radio group with `light`, `dark`, and `system` choices. The selected option is saved in `localStorage` under `portfolio-theme`, and the script applies a `data-theme` attribute to the `<html>` element so the CSS can respond to the selected state. When the user picks system, `data-theme="system"` is kept as the root state, but the CSS can go back to following the system preference. I wrapped the storage logic in `try`/`catch` blocks so the site does not break if `localStorage` is unavailable. The selected state is exposed through the checked radio button and a screen-reader status message, so the control is still understandable for assistive technology. The script is loaded with `defer` from `/js/theme.js` since it isn't an important part of the site.
